...
namespace SqlLite
{
    public class SqlCommands
    {
        ...
        internal IDbCommand LoadAllValuesByPage(string tableName, int pageNumber, int maxRecords, params SqlFilter[] parameters)
        {
            List<string> setString = new List<string>();
            for (int i = 0; i < parameters.Length; i++)
            {
            ...
            var whereStr = (parameters.Count() > 0)?$"where {string.Join(" ", setString)}" : "";
            string commandString = $"SELECT * FROM {tableName} {whereStr} LIMIT {maxRecords} OFFSET {pageNumber} ";
        ...
        internal IDbCommand GetCount(string tableName, params SqlFilter[] parameters)
        {
            List<string> setString = new List<string>();
            for (int i = 0; i < parameters.Length; i++)
            {
